...
def generate_daily_brief(analyzer, prompts):
        ...
        try:
            ...
            txtFileName = f"TMT_Brief_{datetime.now().strftime('%Y-%m-%d')}_raw.txt"
            ...
            try:
                analyzer.detect_technical_terms(analysis)
            except Exception as e:
                print(f"Warning: Technical terms detection failed: {e}")
        
            print("Formatting report...")
            briefs_dir = base_path / 'api' / 'static' / 'assets' / 'briefs'
            filename = format_brief(analysis, briefs_dir)
            return filename
